...
(async()=>{
 ...
 for(const [width,height] of [[1366,768],[1440,900],[1920,1080]]){
  ...
  const closing=await page.locator('#home-solutions').evaluate(e=>e.getBoundingClientRect().toJSON());assert.ok(closing.top<=1);assert.equal(await page.locator('.stack-card').count(),4);assert.equal(await page.locator('.desktop-header').evaluate(e=>e.classList.contains('home-glass')),false);assert.equal(await page.locator('.desktop-header').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(255, 255, 255)');assert.equal(await page.locator('.desktop-header').evaluate(e=>getComputedStyle(e).position),'fixed');
